...
DISTRIBUTION_BUCKET=<optional: public distribution bucket, e.g., www.aws-services.synepho.com>
DISTRIBUTION_KEY=<optional: S3 key path in distribution bucket, default: reports/aws-service-report-latest.xlsx>
NODE_ENV=production
AWS_REGION=us-east-1
```

**Distribution Configuration (Optional):**
- If `DISTRIBUTION_BUCKET` is not set or empty, distribution is skipped
- Distribution copies the latest report with `cache-control: public, max-age=300` (5 minutes)
- Distribution failures are logged as warnings but don't fail report generation
- Useful for serving reports via public S3 website or CloudFront CDN
